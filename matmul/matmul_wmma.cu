...
__global__ void mm(T1 *a, T1 *b, T2 *c, int m, int k, int n) {
  ...
  //(m,k) and (k,n) both are row-major.
  int lda = k, ldb = n;
  //each wrap is responsible for 16 x 16 chunk for output matrix.
  //a_frag
  wmma::fragment<wmma::matrix_a, 16, 16, 16, T1, wmma::row_major> a_frag;
  //b_frag
  wmma::fragment<wmma::matrix_b, 16, 16, 16, T1, wmma::row_major> b_frag;
  //accum_frag
  ...
  for (int i=0; i<k; i+=16) {
    ...
    T1 *a_ptr = a + (a_start_row * k + a_start_col);
    T1 *b_ptr = b + (b_start_row * n + b_start_col);
